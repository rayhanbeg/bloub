...
export function LandingPage() {
  ...
  return (
    <div className="min-h-dvh overflow-hidden bg-[#fcfcfb] text-zinc-900">
      ...
      <main>
        <section className="relative mx-auto grid max-w-6xl items-center gap-10 px-5 pb-20 pt-10 sm:px-8 sm:pb-28 sm:pt-16 lg:grid-cols-[1fr_0.92fr] lg:gap-16 lg:px-10 lg:pb-36 lg:pt-24">
          <div className="relative z-[1] max-w-xl">
            ...
            <motion.h1 initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.06 }} className="mt-5 text-balance text-[clamp(3rem,8vw,6.4rem)] font-semibold leading-[0.89] tracking-[-0.075em] text-zinc-950">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 110, damping: 20, delay: 0.08 }} className="relative mx-auto aspect-square w-full max-w-[480px]">
            ...
            <BlobPreview config={{ shape: 'pebble', mood: 'excited', color: '#111113' }} size="100%" className="relative z-[1]" />
        ...
        <section className="border-y border-zinc-200/80 bg-white">
          <div className="mx-auto grid max-w-6xl gap-px px-5 sm:grid-cols-3 sm:px-8 lg:px-10">
            {[
              ...
              <div key={title} className="border-b border-zinc-200/80 py-8 last:border-b-0 sm:border-b-0 sm:px-7 sm:py-11 sm:first:pl-0 sm:last:pr-0 sm:not(:last-child):border-r">
        ...
        <section className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28 lg:px-10">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div><p className={sectionTitle}>A small cast</p><h2 className="mt-3 text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">Same soul, different energy.</h2></div>
        ...
        <section className="border-t border-zinc-200/80 bg-zinc-950 text-white">
          <div className="mx-auto grid max-w-6xl gap-12 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-[1fr_auto] lg:items-end lg:px-10">
            <div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-zinc-400">Create · Customize · Export</p><h2 className="mt-4 text-5xl font-semibold leading-[0.92] tracking-[-0.065em] sm:text-6xl">Built in the open.</h2><p className="mt-5 max-w-lg text-[15px] leading-relaxed text-zinc-300">Bloub is an open-source creative playground. Explore the code, shape it to your needs, or simply make a character that makes you smile.</p></div>
      ...
      <footer className="bg-zinc-950 text-zinc-400"><div className="mx-auto flex max-w-6xl flex-col gap-3 border-t border-white/10 px-5 py-6 text-[11.5px] sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-10"><span>Developed by Rayhan</span><div className="flex gap-4"><a href={GITHUB} target="_blank" rel="noreferrer" className="hover:text-white">GitHub</a><a href={INSTAGRAM} target="_blank" rel="noreferrer" className="hover:text-white">Instagram</a></div></div></footer>
    </div>
  )
}
